...
RC HeapTableEngine::create_index(Trx *trx, const vector<const FieldMeta *> &field_metas, const char *index_name)
{
  ...
  // 转换为非指针的FieldMeta vector
  vector<FieldMeta> field_meta_vec;
  for (const FieldMeta *field_meta : field_metas) {
    if (nullptr == field_meta) {
      LOG_INFO("Invalid field_meta (nullptr) in table:%s, index_name:%s", table_meta_->name(), index_name);
      return RC::INVALID_ARGUMENT;
    }
  ...
  IndexMeta new_index_meta;
  RC rc = new_index_meta.init(index_name, field_meta_vec);
  if (rc != RC::SUCCESS) {
  ...
  rc = index->create(table_, index_file.c_str(), new_index_meta, field_meta_vec);
  if (rc != RC::SUCCESS) {
    delete index;
    LOG_ERROR("Failed to create bplus tree index. file name=%s, rc=%d:%s", index_file.c_str(), rc, strrc(rc));
    return rc;
  ...
  scanner->close_scan();
  delete scanner;
  LOG_INFO("inserted all records into new multi-column index. table=%s, index=%s, fields:%d", 
           table_meta_->name(), index_name, (int)field_metas.size());
  ...
  LOG_INFO("Successfully added a new multi-column index (%s) on the table (%s)", index_name, table_meta_->name());
  return rc;
}
